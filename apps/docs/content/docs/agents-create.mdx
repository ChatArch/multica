...
`custom_env` values are stored in the Multica server database — they are not "stays on this machine" data. Regular agent list and detail endpoints never return plaintext; only workspace `owner`s and `admin`s can unlock and modify values, and every read or change leaves an audit record. A running agent cannot call the admin endpoints to read another agent's variables.
</Callout>
