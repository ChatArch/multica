...
## Reading, parking, and archiving

The inbox groups notifications **one row per issue** — three comments on the same issue are one row, showing the newest event.

Three states, and the actions that move between them:

- **Unread** — a bold row with a dot. Opening a row marks it read immediately; you don't have to do anything.
- **Read** — still in the list. It stays there until you archive it.
- **Archived** — out of the main list, into **Archived** at the bottom. Archiving is issue-level: it files away every notification for that issue at once.

**Right-click any row** for **Mark as read** / **Mark as unread** and **Archive**. Mark as unread is the "I looked, but I'm not done with this" action — the row goes back to bold and the workspace's unread count rises again, and it stays unread even while you keep it open. Reading it a second time, later, marks it read again.

Archiving preserves read state, so unarchiving brings a notification back exactly as you left it — including unread.
