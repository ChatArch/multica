    ...
    entries: [
      {
        ...
        changes: [],
        features: [
          "指派确认弹窗现在能用发送快捷键完成，不用再动鼠标。",
          "创建智能体用的那段对话会留下来，之后还能回看。",
        ],
        improvements: [
          "创建智能体的页面开着，不再拖慢整个工作区。",
          "创建智能体时填的内容会自动存下来，切走再回来不用重填。",
          "Skill 文件的行菜单里直接有「编辑」，改文件少点两下。",
          "聊天里的快捷建议现在跟着你写的语言给。",
          "智能体每次开工读的说明更短，更多额度花在你的活上。",
          "本地构建的运行时会明确标出来，不再提示一个不存在的更新。",
          "运行时详情页的标题不再重复显示机器名。",
        ],
        fixes: [
          "中断回复后接着发消息，智能体还记得前面聊过的内容。",
          "创建智能体的输入框发送后会清空，边发边写的内容也不会丢。",
          "点 Skill 里的文件现在能打开了，不再只弹出菜单。",
          "Issue 里的「取消订阅」点了真的会取消。",
          "在回复框里输入 / 也能看到工作区的快捷指令。",
          "被 Blocked 的 Issue 一改回其他状态，智能体就会继续干。",
          "从 Issue 头部打开运行中的执行日志，弹窗不再一闪就没。",
          "手机端聊天在实时更新丢了的时候会自己补上。",
          "OpenClaw 写完的回复会立刻送到，任务位置也及时让出来。",
          "在 Slack、飞书里单发 /new，不再多出一条空消息和一次空运行。",
          "新接入的聊天渠道现在能正常收到附件。",
          "超长的项目名或标签名不再把属性标签撑满一整行。",
          "清空某个属性的入口，现在统一是弹层的第一行。",
          "Autopilot 的 Runbook 再宽也不会把设置面板挤出弹窗。",
          "中文侧栏里的 Discord 入口不再显示成半截文字。",
          "把 Issue 指派给 Squad 不再在后台报错。",
        ],
      },
      {
        version: "0.4.17",
