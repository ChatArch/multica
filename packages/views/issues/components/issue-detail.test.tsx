...
describe("IssueDetail (shared)", () => {
  ...
  // Posting a top-level comment ends the turn: rather than keeping the caret in
  // the composer, the page reveals what was just posted — the same scroll +
  // brand flash an inbox deep-link produces. The composer's own side of this
  // (blur, and NOT refocus) is covered in comment-composers.test.tsx.
  describe("reveal after posting a comment", () => {
    it("scrolls to and flashes the comment that was just posted", async () => {
      ...
      fireEvent.change(editor, { target: { value: "just posted" } });

      // Every thread renders its own (disabled, empty) reply Send button; the
      // enabled one is the top-level composer that was just typed into.
      const send = screen
        .getAllByRole("button", { name: "Send" })
        .find((button) => !(button as HTMLButtonElement).disabled);
      fireEvent.click(send!);

      await waitFor(() => expect(mockApiObj.createComment).toHaveBeenCalled());
      // The reveal is deferred two frames (composer clear + cache commit), so
      // the flash lands after the row itself does.
      await waitFor(() => {
        expect(
          hasHighlightedCommentBackground(document.getElementById("comment-comment-new")),
        ).toBe(true);
      });
    });

    it("does not flash anything when the post fails", async () => {
      mockApiObj.createComment.mockRejectedValue(new Error("nope"));
      renderIssueDetail();
      ...
      await new Promise((resolve) => requestAnimationFrame(() => resolve(null)));
      // Nothing new to reveal — and the pre-existing rows must stay unflashed.
      expect(
