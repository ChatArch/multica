...
func init() {
	...
	issueCommentListCmd.Flags().Int("recent", 0, "Return the N most recently active threads. N caps THREADS, not comments: every thread carries its root plus EVERY descendant with no per-thread cap, so on an issue with fewer than N root threads this returns the entire history (minus folded resolved threads). Prefer --roots-only + --thread --tail for bounded reads. Use --before/--before-id from the previous response to scroll to older threads.")
