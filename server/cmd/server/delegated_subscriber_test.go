...
func TestDelegatedTier_ChildCompletionDeliversExactlyOnce(t *testing.T) {
	...
	notifySubscribers(ctx, inboxv2.NewWriter(testPool, queries), queries, bus, childID, "in_progress", testWorkspaceID,
		agentEvent, nil, "status_changed", "info", "child", "", emptyDetails,
		fieldChangeDelivery(inboxv2.TypeStatusChanged, handler.IssueResponse{ID: childID}, "status", agentEvent))
	if n := countInbox(); n != 0 {
	...
	notifySubscribers(ctx, inboxv2.NewWriter(testPool, queries), queries, bus, childID, "in_review", testWorkspaceID,
		agentEvent, nil, "status_changed", "info", "child", "", emptyDetails,
		fieldChangeDelivery(inboxv2.TypeStatusChanged, handler.IssueResponse{ID: childID}, "status", agentEvent))
	if n := countInbox(); n != 1 {
