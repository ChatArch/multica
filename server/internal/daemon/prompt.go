...
func buildPromptBody(task Task, provider string) string {
	...
	fmt.Fprintf(&b, "For comment history, follow the rule in your runtime workflow file (assignment-triggered tasks treat the read as mandatory). Scan the threads first with `multica issue comment list %s --roots-only --summary --output json`, then expand only what matters with `--thread <thread-id> --tail 30`. `multica issue comment list %s --recent 10 --output json` pulls the 10 most recently active threads in full — it has no per-thread depth cap, so below 10 threads it returns the entire history; use it only when you need several complete threads at once. Page older threads via the stderr `Next thread cursor: ...` line and the matching `--before` / `--before-id` until you have enough history. Resolved threads come back folded — `--full` to expand. `--since <RFC3339>` is still available for incremental polling and may combine with `--recent`.\n", task.IssueID, task.IssueID)
	return b.String()
}
...
func buildCommentPrompt(task Task, provider string) string {
	...
	if hint := execenv.BuildNewCommentsHint(task.IssueID, task.TriggerCommentID, task.TriggerThreadID, task.NewCommentsSince, task.NewCommentCount); hint != "" {
		...
		fmt.Fprintf(&b, "Read the discussion: scan with `multica issue comment list %s --roots-only --summary --output json`, then expand what matters with `--thread <thread-id> --tail 30`. `multica issue comment list %s --recent 10 --output json` returns those threads in full when you need them (resolved threads come back folded — `--full` to expand).\n\n", task.IssueID, task.IssueID)
	}
