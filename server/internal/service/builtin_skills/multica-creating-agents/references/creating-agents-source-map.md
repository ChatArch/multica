...
| No generated avatar default | `agent_avatar.go` `newAgentAvatar`; `agent.go` create path | Omitted, empty, or whitespace-only `avatar_url` stores NULL; an explicit value is routed through `acceptAvatarURL` (same publish check as update) and preserved. The template handler uses the same helper (`agent_template.go`). The UI derives the fallback from the agent's runtime icon at render time, so nothing is snapshot into the row (MUL-5399 replaced the former random `emoji:<glyph>` sentinel; migration 235 cleared the ones already written). |
